...
GSEA_Gage_old <- function(res_all, DEmethod, G = NULL, output_loc, time_point, draw_map = FALSE, num_paths = 15) {
  if(DEmethod == "DESeq2")
  {
  ...
  print(upregulated %>%tbl_df%>% ggplot(aes(y = stat.mean , x = forcats::fct_reorder(substr(rownames(upregulated),start=10, stop=10e2), stat.mean), size = set.size, colour= q.val)) +
          geom_point()+
          labs(x = "Gene set", y = "Enrichment score", colour="q value", size="Count") +   coord_flip() +  theme_bw() + 
  ...
  print(downregulated %>%tbl_df%>% ggplot(aes(y = stat.mean , 
                                              x = forcats::fct_reorder(substr(rownames(downregulated),start=10, 
                                                                              stop=10e2), stat.mean), size = set.size, colour= q.val)) + geom_point()+
          labs(x = "Gene set", y = "Enrichment score", colour="q value", size="Count") +   coord_flip() +  theme_bw() + 
          theme(axis.title = element_text(size = 16, color = "black"),
                axis.text = element_text(size = 16, color = "black") ))
...
# takes in matrix with fold changes and gene names, and returns pathway enrichment figures and list of up- and down-regulated genes
GSEA_Gage <- function(count_data, ctrt_idx, case_idx, gene_set_kegg = kegg.gs, output_loc, time_point, num_paths = 10, draw_map = FALSE) 
{
  if (length(ctrt_idx) == length(case_idx))
    {
  keggres <- gage(count_data, gsets = gene_set_kegg,
                  ref = ctrt_idx, samp = case_idx)
  }
  else
  {
  keggres <- gage(count_data, gsets = gene_set_kegg,
                    ref = ctrt_idx, samp = case_idx, compare = "unpaired")
  }
  lapply(keggres, head)
  ...
  result <- list("upregulated" = keggres$greater, "downregulated" = keggres$less) 
  return(result) 
...
compare_chip_Zaas <- function(enriched_paths_chip, enriched_paths_flu, direction = "upregulated",num_paths=20, output_name)
  {
  if(direction == "upregulated")
  {
    enriched_paths_cp <-enriched_paths_chip$upregulated[,c(1:5)]
    enriched_paths_fl <- enriched_paths_flu$upregulated[,c(1:5)]
  }
  else
  {
    enriched_paths_cp <-enriched_paths_chip$downregulated[,c(1:5)]
    enriched_paths_fl <- enriched_paths_flu$downregulated[,c(1:5)]
  }
  g1 <- enriched_paths_cp %>%tbl_df %>% dplyr::slice(1:num_paths)
  g2 <- enriched_paths_fl %>%tbl_df%>% dplyr::slice(1:num_paths)
  ...
  g4 <- rbind(g1, g2)
  g4 <- add_column(g4, pathways = c(substr(rownames(enriched_paths_cp)[1:num_paths], 
                                           start=10, stop=10000),
                                    substr(rownames(enriched_paths_fl)[1:num_paths], 
                                           start=10, stop=10000)))
  print(g4 %>% 
  ...
  ggsave(filename = paste(output_name, "_",direction,".pdf",sep=""), 
         plot = last_plot(), 
         device = "pdf", 
         height = 8,
         width = 10,
         scale = 1, 
         dpi = 400)
  ggsave(filename = paste(output_name, "_",direction,".png",sep=""), 
         plot = last_plot(), 
         device = "png", 
         height = 8,
         width = 10,
...
##------------------------------------------------------------------------------------------------------ ##
## Compare paths GAGE (Zaas and chip)
compare_chip_copd_Zaas <- function(enriched_paths_chip, enriched_paths_flu, enriched_paths_copd, direction = "upregulated",num_paths=20, output_name)
{
  if(direction == "upregulated")
  {
    enriched_paths_cp <-enriched_paths_chip$upregulated[,c(1:5)]
    enriched_paths_fl <- enriched_paths_flu$upregulated[,c(1:5)]
    enriched_paths_copd <- enriched_paths_copd$upregulated[,c(1:5)]
  }
  else
  {
    enriched_paths_cp <-enriched_paths_chip$downregulated[,c(1:5)]
    enriched_paths_fl <- enriched_paths_flu$downregulated[,c(1:5)]
    enriched_paths_copd <- enriched_paths_copd$downregulated[,c(1:5)]
    
  }
  g1 <- enriched_paths_cp %>%tbl_df %>% dplyr::slice(1:num_paths)
  g1b <- enriched_paths_copd %>%tbl_df %>% dplyr::slice(1:num_paths)
  g2 <- enriched_paths_fl %>%tbl_df%>% dplyr::slice(1:num_paths)
  
  g1 <- g1 %>% 
    tibble::add_column(., group = "chip (nhb)") %>% 
    tibble::add_column(., rank = seq(1, nrow(g1))) %>%
    dplyr::mutate(., norm_score = (stat.mean - min(stat.mean)) / (max(stat.mean) - min(stat.mean)))
  
  g1b <- g1b %>% 
    tibble::add_column(., group = "chip (copd)") %>% 
    tibble::add_column(., rank = seq(1, nrow(g1))) %>%
    dplyr::mutate(., norm_score = (stat.mean - min(stat.mean)) / (max(stat.mean) - min(stat.mean)))
  ...
  g4 <- rbind(g2, g1, g1b)
  g4 <- add_column(g4, pathways = c(substr(rownames(enriched_paths_fl)[1:num_paths], 
                                           start=10, stop=10000),
                                    substr(rownames(enriched_paths_cp)[1:num_paths], 
                                           start=10, stop=10000),
                                    substr(rownames(enriched_paths_copd)[1:num_paths], 
                                           start=10, stop=10000)))
  print(g4 %>% 
          #dplyr::filter(., group != "network data") %>%
          ggplot() + 
          geom_point(aes(x = forcats::fct_reorder(pathways, stat.mean), y = stat.mean, color = group, size = set.size)) +
          scale_color_manual(values = c('deeppink', 'darkred','green', 'blue', 'orange')) + # , "#99cc99"
          labs(x = "Gene set", y = "Enrichment score") +
          coord_flip() + 
          facet_grid(. ~ group, scales = "free") +
          theme_bw() + 
...
plot_volcano <- function(res, x_min, x_max, pcutoff = 0.05, output_name, points_to_highlight=NULL)
{
  ...
  p1<-EnhancedVolcano(res,
                  ...
                  x = 'log2FoldChange',
                  y = 'padj',
                  selectLab = res$gene[which(res$gene %in% points_to_highlight)],
                  pCutoff = pcutoff,
                  FCcutoff = 1.0,
                  xlim = c(x_min, x_max),
                  col=c('black', '#4D98FA', 'orange', 'hotpink'),
                  colAlpha = 1.0,
                  pointSize =  2.5,
                  labSize = 6,
    ...
    p1+ggplot2::coord_cartesian(xlim=c(-4, 8))+
      xlim(-4, 8)+ggplot2::coord_cartesian(ylim=c(0, 12.5))+ggplot2::scale_x_continuous(
      breaks=seq(-2,7, 2), limits = c(-2.5,7))+ggplot2::scale_y_continuous(
        breaks=seq(0,12.5, 2))
